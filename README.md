...
- **Review step**: with "Add to cart" off (default) the customer sends an enquiry — the contact form's SKU and Design number fields are prefilled with the setting/metal/size and the stone's certificate. Turning it on needs the API's `/cart` endpoint (creates the diamond as a hidden product and returns its variant id) — not built yet.
