...
- **Review step**: "Add to cart" (or "Pay 25% deposit" when a deposit is configured) appears when the Builder API reports `cart: true` on `/api/health` — it authenticates to Shopify with the Dev Dashboard app's client credentials, so there is no long-lived token. The theme POSTs the build to `/api/cart`; the API re-checks the stone with Nivoda, prices it server-side and creates a hidden Shopify product, published to the Online Store only, tagged `hidden-service` + `ring-builder` + `build:<ref>` + `stone:<cert>`. In **deposit mode** (`DEPOSIT_PCT=25` on the API, the current setting) that product is one combined "Ring build" line priced at the deposit, with Ring total / Deposit / Balance on completion as line properties; with no deposit the API creates **the diamond as its own product** (type "Diamond", SKU = certificate number) and the theme adds two lines — the setting's own variant plus the diamond — sharing the build ID, setting, stone, finger size and lead time as line properties. Builds nobody buys are archived after 7 days by the API. Verified end to end on 9 Sep 2026 (build `RB-MTTG13DL`, archived afterwards). Without cart access the customer sends an enquiry instead — the contact form's SKU and Design number fields are prefilled with the setting/metal/size and the stone's certificate.
- **Settings catalogue (9 Sep 2026)**: the six `mount-sample-*` products now carry everything the old VDB/Nivoda builder had — its "288 rings" were these same six Smiling Rocks semi-mounts listed per centre-stone size. Each product has the old title, 10 metal variants (9/14/18ct × white/yellow/rose + platinum; 14ct = the old builder's AUD price, the others placeholders), `builder.price_by_carat` (the old 14ct price for each centre size 0.30–5ct; the theme and API scale the chosen variant by bracket ÷ 1ct), and ~140 renders keyed `Colour|Shape|Size` (plus the size-less `Colour|Shape` set used before a stone is chosen). Images and designs remain Smiling Rocks' — a supplier-terms question before going live, and still to be replaced by the CAD designer's catalogue.
